...
##specifically comparing the number of rows of each table to the number of rows in the first column of the table that has the word "Demos" in it -- expecting them to be equal
##unlist together with which grabs the index of the table that has the Demos in it
which_table <- which(unlist(lapply(1:length(rendered_tables), function(i) nrow(rendered_tables[[i]]) == sum(str_detect(unlist(rendered_tables[[i]][,1]), "Demos")))))

knitr::kable(rendered_tables[[which_table]])
```
