...
Use the Start-up script found in the Data tab, under other data there is a workspace option. While creating an RStudio Cloud Environment on AnVIL, paste the path below into the Startup script *Optional box* to install `AnVILGCP`, `vcfR`, and `bcftools`.
...
We will load the three required R packages (`tidyverse`, `vcfR`, `AnVILGCP`) and receive an introduction to how R packages work, including the distinction between installation and loading.
